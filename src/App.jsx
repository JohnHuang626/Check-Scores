...
const ComparisonBar = ({
  ...
  const displayScore =
    isCap && typeof myScore === 'string' ? CAP_POINTS[myScore] || 0 : myScore;
  ...
  return (
    <div className="mb-3">
      <div className="flex justify-between text-xs mb-1">
        ...
        <div className="space-x-2">
          <span className="text-blue-600 font-bold">
            我: {myScore} {isCap ? `(${displayScore}分)` : ''}
          </span>
      ...
      <div className="h-2 bg-gray-100 rounded-full overflow-hidden relative">
        <div
          className="absolute top-0 bottom-0 bg-gray-400 w-1 z-10 opacity-50"
          style={{ left: `${(numAvg / maxScore) * 100}%` }}
        ></div>
        <div
          className={`h-full rounded-full transition-all duration-500 ${
            displayScore >= numAvg ? 'bg-blue-500' : 'bg-orange-400'
          }`}
...
const TrendChart = ({ data, title }) => {
  if (!data || data.length === 0)
    return <div className="text-gray-400 text-sm">尚無足夠數據繪製趨勢圖</div>;
      ...
      >
        {[0, 0.25, 0.5, 0.75, 1].map((r) => (
          <line
            key={r}
            x1={padding}
            y1={height - padding - r * (height - 2 * padding)}
            x2={width - padding}
            y2={height - padding - r * (height - 2 * padding)}
            stroke="#eee"
            strokeWidth="1"
          />
        ))}
...
export default function App() {
  ...
  const initializeDatabase = async () => {
    ...
    if (
      !confirm(
        '確定要寫入測試資料嗎？這將會新增範例學生與成績到您的 Firebase。'
      )
    )
      return;
    ...
    try {
      ...
      console.error(error);
      alert(
        '寫入失敗。請檢查您的 Firebase Console 權限設定 (Security Rules) 是否為 Test Mode。'
      );
    }
  };
  ...
  const calculateStats = (scores, category) => {
    ...
    if (category === 'regular') {
      const social = parseFloat(
        (
          ((scores.geo || 0) + (scores.his || 0) + (scores.civ || 0)) /
          3
        ).toFixed(1)
      );
  ...
  const handleLogin = (role, account, password) => {
    if (role === 'teacher') {
      ...
      const isDefaultLogin =
        (connectionStatus === 'demo' || students.length === 0) &&
        account === DEFAULT_TEACHER.account &&
        password === DEFAULT_TEACHER.password;

      if (
        isDefaultLogin ||
        (account === teacherProfile.account &&
      ...
      const student = students.find(
        (s) => s.account === account && s.password === password
      );
      if (student) {
        setUserRole('parent');
        ...
        setIsLoggedIn(true);
        setActiveTab('dashboard');
        const lastExamId = Object.keys(gradesDB)
          .reverse()
          .find((eid) => gradesDB[eid] && gradesDB[eid][student.id]);
  ...
  const handleUpdateStudent = async () => {
    if (db) {
      ...
      if (editingStudentId === 'new') {
        setStudents([
          ...students,
          { ...tempStudentData, id: Date.now().toString() },
        ]);
      } else {
        setStudents(
          students.map((s) => (s.id === editingStudentId ? tempStudentData : s))
        );
      }
    }
  ...
  const handleGradeChange = async (studentId, subject, value) => {
    ...
    if (db) {
      ...
      await setDoc(
        ...
        {
          [studentId]: {
            ...(gradesDB[teacherExamId]?.[studentId] || {}),
            [subject]: finalValue,
          },
        },
        { merge: true }
  ...
  const handleDownloadGradeTemplate = () => {
    ...
    let headers =
      currentCategory === 'regular'
        ? '座號,姓名,國文,英語,數學,自然,地理,歷史,公民'
        : '座號,姓名,國文,英語,數學,自然,社會';
    let example =
      currentCategory === 'regular'
        ? '6,王小明,80,85,90,88,85,82,88'
        : '6,王小明,A++,A,B++,A+,B';
  ...
  const handleGradeUpload = (event) => {
    ...
    reader.onload = async (e) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (student) {
          ...
          if (currentCategory === 'regular' && parts.length >= 9) {
            scores.chi = Number(parts[2]);
            scores.eng = Number(parts[3]);
            scores.math = Number(parts[4]);
            scores.sci = Number(parts[5]);
            scores.geo = Number(parts[6]);
            scores.his = Number(parts[7]);
            scores.civ = Number(parts[8]);
          } else if (currentCategory === 'cap' && parts.length >= 7) {
            scores.chi = parts[2].trim().toUpperCase();
            scores.eng = parts[3].trim().toUpperCase();
            scores.math = parts[4].trim().toUpperCase();
            scores.sci = parts[5].trim().toUpperCase();
            scores.soc = parts[6].trim().toUpperCase();
          }
          if (Object.keys(scores).length > 0) {
            updates[student.id] = {
              ...(gradesDB[teacherExamId]?.[student.id] || {}),
              ...scores,
            };
            count++;
          }
      ...
      if (count > 0) {
        if (db)
          await setDoc(doc(db, 'grades', teacherExamId), updates, {
            merge: true,
          });
        else
          setGradesDB((prev) => ({
            ...prev,
            [teacherExamId]: { ...prev[teacherExamId], ...updates },
          }));
  ...
  const currentExamData = useMemo(() => {
    ...
    const allStudentStats = students
      .map((s) => {
    ...
    const avgTotal =
      allStudentStats.length > 0
        ? (
            allStudentStats.reduce((a, b) => a + b, 0) / allStudentStats.length
          ).toFixed(1)
        : 0;
    const subjects =
      category === 'regular'
        ? ['chi', 'eng', 'math', 'sci', 'geo', 'his', 'civ']
        : ['chi', 'eng', 'math', 'sci', 'soc'];
    const classAvgs = {};
  ...
  const trendData = useMemo(() => {
    ...
    const availableExams = EXAM_OPTIONS.filter(
      (opt) =>
        gradesDB[opt.id] &&
        gradesDB[opt.id][currentUser.id] &&
        opt.category === currentCat
    );
  ...
  const teacherTableData = useMemo(() => {
    ...
    if (sortConfig.key) {
      data.sort((a, b) => {
        let aValue, bValue;
        if (
          ['chi', 'eng', 'math', 'sci', 'geo', 'his', 'civ', 'soc'].includes(
            sortConfig.key
          )
        ) {
          if (currentCategory === 'cap') {
            aValue = CAP_POINTS[a.scores[sortConfig.key]] || 0;
            bValue = CAP_POINTS[b.scores[sortConfig.key]] || 0;
          } else {
    ...
    return (
      <div className="min-h-screen bg-blue-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md relative">
          <div
            className={`absolute top-4 right-4 text-xs font-bold px-2 py-1 rounded-full flex items-center ${
          ...
          <div className="text-center mb-6">
            <h1 className="text-2xl font-bold text-gray-800">
              {teacherProfile.className} 成績查詢系統
            </h1>
          ...
          {connectionStatus === 'connected' && students.length === 0 && (
            <div className="mt-6 pt-6 border-t border-gray-100">
              <div className="bg-orange-50 border border-orange-100 rounded-lg p-3 mb-3 text-xs text-orange-700 flex items-start">
                ...
                <div>
                  <p className="font-bold">資料庫目前是空的</p>
                  <p>請點擊下方按鈕初始化測試資料。</p>
                </div>
              </div>
              <button
                onClick={initializeDatabase}
                className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 rounded-lg transition-colors text-sm flex items-center justify-center"
              >
                <Database size={16} className="mr-2" /> 初始化測試資料 (寫入
                Firebase)
              </button>
            </div>
          )}
          ...
          {connectionStatus === 'demo' && (
            <div className="mt-4 text-xs text-gray-400 text-center">
              ...
              <p>
                若要上線，請在 StackBlitz 執行 <code>npm install firebase</code>{' '}
                並設定 Config。
              </p>
            </div>
          )}
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      ...
      <div
        ...
        } md:relative md:translate-x-0 transition-transform w-64 bg-white shadow-lg z-40 flex flex-col print:hidden`}
      >
        <div className="p-6 border-b">
          ...
          <p className="text-sm text-gray-500 mt-1">
            你好，{currentUser.name} {userRole === 'teacher' ? '老師' : ''}
          </p>
        </div>
        ...
        <div className="p-4 border-t">
          <button
            ...
            className="flex items-center space-x-3 text-red-500 p-2"
          >
            <LogOut size={20} />
            <span>登出</span>
          </button>
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto h-screen print:h-auto print:overflow-visible">
        ...
        {userRole === 'teacher' && activeTab === 'settings' && (
          <div className="max-w-2xl mx-auto">
            <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center">
              <Settings className="mr-2 text-blue-600" /> 班級與導師設定
            </h2>
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
              <form onSubmit={handleUpdateProfile} className="space-y-6">
                <div>
                  ...
                  <input
                    name="className"
                    defaultValue={teacherProfile.className}
                ...
                <div className="pt-4 border-t border-gray-100">
                  <button
                    type="submit"
                    className="w-full bg-blue-600 text-white font-bold py-3 rounded-lg hover:bg-blue-700 flex justify-center items-center"
                  >
        ...
        {userRole === 'teacher' && activeTab === 'students' && (
          <div className="max-w-5xl mx-auto">
            <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center">
              <Users className="mr-2 text-blue-600" /> 學生資料與帳號管理
            </h2>
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="p-4 border-b border-gray-100 bg-gray-50 flex flex-col md:flex-row justify-between items-center gap-4">
                <span className="font-bold text-gray-700">
                  學生列表 ({students.length}人)
                </span>
                <div className="flex gap-2">
                  <button
                    onClick={handleDownloadStudentTemplate}
                    className="bg-gray-100 text-gray-700 px-3 py-2 rounded-lg text-sm flex items-center hover:bg-gray-200"
                  >
                    <Download size={16} className="mr-1" /> 下載範本
                  </button>
                  <label className="bg-blue-100 text-blue-700 px-3 py-2 rounded-lg text-sm flex items-center hover:bg-blue-200 cursor-pointer">
                  ...
                  <button
                    ...
                    className="bg-green-600 text-white px-3 py-2 rounded-lg text-sm flex items-center hover:bg-green-700"
                  >
                    ...
                    <tr className="bg-green-50">
                      ...
                      <td className="p-4">
                        <input
                          className="w-32 border rounded p-1"
                          placeholder="密碼"
                          value={tempStudentData.password}
                          onChange={(e) =>
                            setTempStudentData({
                              ...tempStudentData,
          ...
          <div className="w-full space-y-4">
            <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100 print:hidden">
              ...
              <div className="mt-4 md:mt-0 flex flex-wrap gap-2">
                ...
                <label className="flex items-center space-x-1 bg-blue-100 text-blue-700 px-3 py-2 rounded-lg hover:bg-blue-200 cursor-pointer">
                  <Upload size={16} />
                  <span>批次匯入</span>
                  <input
                    type="file"
                ...
                <button
                  ...
                  className="flex items-center space-x-1 bg-gray-800 text-white px-3 py-2 rounded-lg hover:bg-gray-700"
                >
                  <Printer size={16} />
                  <span>列印</span>
                </button>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden print:border-none">
              <div className="hidden print:block text-center p-4">
                <h1 className="text-2xl font-bold">
                  {EXAM_OPTIONS.find((e) => e.id === teacherExamId)?.label}{' '}
                  成績一覽表
                </h1>
                <p className="text-gray-600">
                  {teacherProfile.className} | 導師: {teacherProfile.name}
                </p>
                      ...
                      {currentTeacherExamCategory === 'regular' ? (
                        <>
                          <SortableHeader
                            label="地理"
                            sortKey="geo"
                            className="text-center bg-yellow-50/50"
                          />
                          <SortableHeader
                            label="歷史"
                            sortKey="his"
                          ...
                          <SortableHeader
                            label="社會均"
                            sortKey="social"
                            className="text-center font-bold text-yellow-800 bg-yellow-100/50"
                          />
                          <SortableHeader
                            label="總分"
                            sortKey="total"
                            className="text-center font-bold text-blue-800 bg-blue-50"
                          />
                        </>
                      ) : (
                        <>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {teacherTableData.map((student) => (
                      <tr key={student.id} className="hover:bg-gray-50">
                        ...
                        {(currentTeacherExamCategory === 'regular'
                          ...
                          <td key={sub} className="p-1">
                            {currentTeacherExamCategory === 'regular' ? (
                              <input
                                type="number"
                                className="w-full text-center p-1 border border-transparent hover:border-blue-300 focus:border-blue-500 rounded bg-transparent focus:bg-white"
                              ...
                              <select
                                className="w-full text-center p-1 border border-transparent hover:border-blue-300 focus:border-blue-500 rounded bg-transparent focus:bg-white appearance-none cursor-pointer"
                                value={student.scores[sub] || ''}
                                onChange={(e) =>
                                  handleGradeChange(
                                    student.id,
                        ...
                        {currentTeacherExamCategory === 'regular' ? (
                          <>
                            <td className="p-3 text-center font-bold text-yellow-700 bg-yellow-50/30">
                              {student.social}
                            </td>
                ...
                <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white rounded-xl shadow-lg p-6 relative overflow-hidden">
                  <div className="relative z-10">
                    <p className="text-blue-100 text-sm">
                      {currentExamData.category === 'regular'
                        ? '本次總分 (500滿分)'
                        : '會考總積分 (35滿分)'}
                    </p>
                    <h3 className="text-5xl font-bold mt-2">
                      {currentExamData.myScores.total}
                    </h3>
                    <div className="mt-4 flex items-center space-x-2">
                      <span className="bg-white/20 px-2 py-1 rounded text-xs">
                        班排 #{currentExamData.rank}
                      </span>
                      <span className="text-xs text-blue-100">
                        / {currentExamData.totalStudents} 人
                      </span>
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col justify-between">
                  ...
                  <div className="mt-4">
                    ...
                    <div className="h-2 bg-gray-100 rounded-full relative">
                      <div
                        className="absolute w-3 h-3 bg-blue-500 rounded-full top-1/2 transform -translate-y-1/2 -translate-x-1/2 border-2 border-white shadow"
                        style={{
                          left: `${Math.min(
                            (currentExamData.myScores.total /
                              (currentExamData.category === 'regular'
                    ...
                    <div className="flex justify-between text-[10px] text-gray-400 mt-1">
                      <span>0</span>
                      <span>
                        {currentExamData.category === 'regular' ? 500 : 35}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    {(currentExamData.category === 'regular'
                      ...
                      return (
                        <ComparisonBar
                          key={subKey}
                          subject={labels[subKey]}
                          myScore={
                            currentExamData.myScores[subKey] ||
                            (currentExamData.category === 'cap' ? 'C' : 0)
                          }
                          avgScore={currentExamData.classAvgs[subKey] || 0}
                          maxScore={
                            currentExamData.category === 'regular' ? 100 : 7
                          }
                          isCap={currentExamData.category === 'cap'}
                        />
                      );
                    })}
                  </div>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 text-gray-500">
                      <tr>
                        ...
                        <th className="p-3 text-center">成績</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {(currentExamData.category === 'regular'
                        ...
                        <tr key={sub.k}>
                          ...
                          <td className="p-3 text-center font-bold text-blue-600">
                            {currentExamData.myScores[sub.k] || '-'}
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          <div className="flex flex-col h-full space-y-6">
            <div className="bg-gradient-to-r from-gray-800 to-gray-700 rounded-2xl p-8 text-white shadow-lg relative overflow-hidden">
              <div className="relative z-10 flex items-center justify-between">
                <div>
                  <p className="text-gray-300 mb-1 font-bold tracking-wide text-sm">
                    TEACHER CONSOLE
                  </p>
                  <h1 className="text-3xl font-bold mb-2">
                    {teacherProfile.className} 班級管理
                  </h1>
                  <div className="flex items-center space-x-2 text-gray-200">
                    <User size={18} />
                    <span className="font-medium">
                      導師：{teacherProfile.name} 老師
                    </span>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <button
                onClick={() => setActiveTab('students')}
                className="p-6 bg-white shadow-sm border border-gray-100 rounded-xl hover:shadow-md transition-all text-left"
              >
              ...
              >
                ...
                <p className="text-sm text-gray-500">
                  支援段考(百分制)與模考(會考積分制)輸入，提供批次匯入功能。
                </p>
              </button>
            </div>
          </div>
